...
class Chef:
    def __init__(self, center_x: float, center_y: float, texture=None):
        ...
        self.texture = texture  # опционально: изображение повара любого разрешения

        # Размер отрисовки (для текстуры масштабируется с сохранением пропорций)
        self.body_width = 70
        self.body_height = 90
        self.head_radius = 18
    ...
    def draw(self):
        ...
        if self.texture is not None:
            # Рисуем текстуру: вписываем в body_width x body_height с сохранением пропорций
